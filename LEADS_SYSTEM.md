# Lead Management System

## ✅ Что реализовано

### 1. База данных PostgreSQL (Neon)
- **URL**: `postgresql://neondb_owner:npg_A9z2dErbkfhw@ep-bitter-frog-a1bewei7-pooler.ap-southeast-1.aws.neon.tech/neondb`
- **Schema**: `schema.sql` - полная схема для лидов

### 2. Таблицы

#### `leads` - Основная таблица лидов
```sql
- id (BIGSERIAL PRIMARY KEY)
- telegram_user_id (BIGINT) - ID пользователя в Telegram
- username, first_name, last_name, phone
- first_message - первое сообщение клиента
- source_chat_id, source_chat_name - откуда пришёл
- status - new/contacted/qualified/converted/lost
- priority - low/medium/high/urgent
- intent - buy/sell/exchange/info
- crypto_interest[] - массив криптовалют
- trigger_words[] - сработавшие триггеры
- agent_response - ответ агента
- total_messages - количество сообщений
- last_activity - последняя активность
```

#### `lead_messages` - История сообщений
```sql
- lead_id - ссылка на лид
- message_text - текст сообщения
- direction - incoming/outgoing
- contains_question - содержит вопрос?
- contains_urgency - содержит срочность?
```

#### `lead_actions` - Лог действий
```sql
- lead_id
- action_type - message_sent/forwarded/status_changed
- action_data - JSON с деталями
```

#### `lead_notes` - Заметки менеджеров
```sql
- lead_id
- note - текст заметки
- note_type - general/important/follow_up
```

### 3. Автоматический анализ лидов

#### Определение намерения (Intent)
- **buy** - куплю, купить, хочу купить
- **sell** - продам, продать
- **exchange** - обмен, обменять, п2п
- **info** - остальное

#### Определение криптовалют
- **Bitcoin** - биткоин, bitcoin, btc
- **USDT** - usdt, тезер, tether
- **Ethereum** - эфир, ethereum, eth

#### Определение приоритета
- **urgent** 🔴 - срочно, быстро, сейчас
- **high** 🟠 - много, большую сумму
- **medium** 🟡 - остальное

### 4. Логирование лидов

**Файл**: `gleam/src/vibee/leads/lead_logger.gleam`

При каждом новом лиде выводится:

```
============================================================
🔥 НОВЫЙ ЛИД СОХРАНЁН
============================================================

📱 КОНТАКТНАЯ ИНФОРМАЦИЯ:
  Telegram ID: 123456789
  Username: @user
  Имя: Иван
  Фамилия: Петров

💬 ПЕРВОЕ СООБЩЕНИЕ:
  куплю биткоин срочно

📊 АНАЛИЗ:
  Намерение: Покупка
  Интересует: Bitcoin
  Приоритет: 🔴 Срочный

🎯 ТРИГГЕРЫ:
  • куплю биткоин

✅ ОТВЕТ АГЕНТА:
  Привет! Могу помочь с покупкой. Пиши в личку, всё расскажу.

📍 ИСТОЧНИК:
  Чат: Aimly.io dev
  ID: -5082217642

============================================================
```

## 📊 Как работает

### Flow

1. **Пользователь пишет триггер**
   ```
   куплю биткоин
   ```

2. **Агент детектит и отвечает**
   ```
   Привет! Могу помочь с покупкой. Пиши в личку.
   ```

3. **Диалог пересылается в Lead группу**
   ```
   🔥 НОВЫЙ ЛИД

   👤 Клиент: User
   💬 Вопрос: куплю биткоин
   ...
   ```

4. **Лид логируется в консоль**
   ```
   [LEAD] 💾 Lead would be saved here
   ```

5. **TODO: Лид сохраняется в PostgreSQL**
   - Вся информация о клиенте
   - История сообщений
   - Анализ намерений
   - Приоритет

## 🔧 Setup PostgreSQL

### 1. Создать таблицы

```bash
# Подключиться к базе
psql 'postgresql://neondb_owner:npg_A9z2dErbkfhw@ep-bitter-frog-a1bewei7-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require'

# Выполнить schema.sql
\i schema.sql
```

### 2. Проверить таблицы

```sql
-- Список таблиц
\dt

-- Структура таблицы leads
\d leads

-- Просмотр лидов
SELECT * FROM active_leads;

-- Статистика
SELECT * FROM lead_stats;
```

### 3. Добавить в .env

```bash
DATABASE_URL=postgresql://neondb_owner:npg_A9z2dErbkfhw@ep-bitter-frog-a1bewei7-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require
```

## 📈 Queries для анализа

### Активные лиды

```sql
SELECT 
    telegram_user_id,
    full_name,
    first_message,
    status,
    priority,
    intent,
    crypto_interest,
    total_messages,
    last_activity
FROM active_leads
ORDER BY priority DESC, last_activity DESC
LIMIT 20;
```

### Лиды за сегодня

```sql
SELECT 
    COUNT(*) as total,
    COUNT(CASE WHEN status = 'new' THEN 1 END) as new,
    COUNT(CASE WHEN priority = 'urgent' THEN 1 END) as urgent
FROM leads
WHERE created_at >= CURRENT_DATE;
```

### Топ криптовалют

```sql
SELECT 
    UNNEST(crypto_interest) as crypto,
    COUNT(*) as count
FROM leads
GROUP BY crypto
ORDER BY count DESC;
```

### Конверсия по статусам

```sql
SELECT 
    status,
    COUNT(*) as count,
    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) as percentage
FROM leads
GROUP BY status
ORDER BY count DESC;
```

## 🚀 TODO: Полная интеграция

### Phase 1: Базовое сохранение ✅
- [x] Schema создана
- [x] Lead logger реализован
- [x] Анализ намерений работает
- [ ] Подключение к PostgreSQL

### Phase 2: Полная интеграция
- [ ] Сохранение в PostgreSQL через pog
- [ ] Получение username из Telegram API
- [ ] Сохранение истории сообщений
- [ ] Логирование всех действий

### Phase 3: Dashboard
- [ ] Web интерфейс для просмотра лидов
- [ ] Фильтры по статусу/приоритету
- [ ] Экспорт в CSV/Excel
- [ ] Уведомления о новых лидах

### Phase 4: CRM интеграция
- [ ] Webhook для внешних CRM
- [ ] API для получения лидов
- [ ] Автоматическое обогащение данных
- [ ] Scoring лидов

## 📞 Использование

### Просмотр лидов в логах

```bash
# Локально
cd /workspaces/vibee-gleam/gleam
export $(cat .env | xargs)
gleam run | grep "НОВЫЙ ЛИД" -A 30

# На Fly.io
fly logs --app vibee-mcp | grep "НОВЫЙ ЛИД" -A 30
```

### Просмотр в базе данных

```bash
# Подключиться
psql 'postgresql://neondb_owner:npg_A9z2dErbkfhw@ep-bitter-frog-a1bewei7-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require'

# Посмотреть лиды
SELECT * FROM active_leads;
```

## 📊 Метрики

### Что отслеживается

1. **Контактная информация**
   - Telegram ID (обязательно)
   - Username (если есть)
   - Имя/Фамилия
   - Телефон (если предоставлен)

2. **Поведение**
   - Первое сообщение
   - Все последующие сообщения
   - Время ответа агента
   - Последняя активность

3. **Намерения**
   - Что хочет (buy/sell/exchange)
   - Какая криптовалюта
   - Приоритет (urgent/high/medium)

4. **Источник**
   - Из какого чата
   - Какие триггеры сработали
   - Ответ агента

## 🎯 Следующие шаги

1. **Сегодня**
   - [x] Schema создана
   - [x] Lead logger работает
   - [ ] Создать таблицы в Neon

2. **Эта неделя**
   - [ ] Подключить pog для PostgreSQL
   - [ ] Сохранять лиды в базу
   - [ ] Dashboard для просмотра

3. **Следующий месяц**
   - [ ] CRM интеграция
   - [ ] Автоматический scoring
   - [ ] Уведомления менеджерам

---

**Status**: Schema ready, logging implemented, PostgreSQL integration pending
**Last Updated**: 2025-12-18 05:18 UTC
