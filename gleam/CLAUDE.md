...
## КРИТИЧЕСКОЕ ПРАВИЛО: NO HARDCODE - Данные в БД!

**ВСЕ КОНФИГУРАЦИОННЫЕ ДАННЫЕ ДОЛЖНЫ ХРАНИТЬСЯ В БАЗЕ ДАННЫХ, НЕ В КОДЕ!**

| Данные | Где хранить | НЕ хранить |
|--------|-------------|------------|
| Триггерные слова | `trigger_words` table | Хардкод в .gleam |
| Настройки чатов | `trigger_chats` table | Хардкод в .gleam |
| Пользовательские настройки | БД | Код |
| API ключи | Fly.io secrets | Код |

### Почему это важно:
1. **Изменения без деплоя** - можно менять триггеры через API
2. **Масштабируемость** - легко добавлять новые триггеры
3. **Аудит** - история изменений в БД
4. **A/B тестирование** - разные триггеры для разных групп

### Правило:
- Если данные могут меняться → **храни в БД**
- Если данные статичны и не меняются → можно в коде (но лучше в БД)
